...
TAG_VERSION="v1.3.4"
VERSION="v1.3.4"
WORKING_DIR="skypool-miner"
MINER_DIR="skypool-nimiq-${VERSION}-linux-x64"

print_logo() {
  echo "Welcome to Skypool.org"
}
...
write_start_background_script() {
  ...
  echo "screen -d -m -S skypool-node-client ${1}" >> "start-background.sh"

  echo "echo \"Skypool Miner has been started in the background.\"" >> "start-background.sh"
  ...
  echo "echo \"screen -r skypool-node-client\"" >> "start-background.sh"
  echo "echo \"\"" >> "start-background.sh"
...
MINER_ZIP_FN="skypool-nimiq-${VERSION}-linux-x64.zip"
MINER_URL="https://github.com/skypool-org/skypool-nimiq-miner/releases/download/${TAG_VERSION}/${MINER_ZIP_FN}"

if [[ -z "$NQ42 EU1Q M6XJ 4R7V AHKJ VEFG A1BH H9YS GRV8" ]]; then
...
echo "Installing Skypool Pool Miner with the following settings:"
...
if [[ -n "$INSTALL_SERVICE" ]]; then
  ...
  CPU_CORES_LINE=""
  if [[ -n "$THREAD" ]]; then
    CPU_CORES_LINE=" --thread=${THREAD}"
  fi
  
  # Generate nonces per run flag
  SERVER_URL_DATA=""
  if [[ -n "$SERVER_URL" ]]; then
    SERVER_URL_DATA=" --server=${SERVER_URL}"
  fi

  # Generate extra data flag
  EXTRADATA=""
  if [[ -n "$WORKER_ID" ]]; then
    EXTRADATA=" --name=\"${WORKER_ID}\""
  fi

  # Write two files; start-foreground.sh / start-background.sh
  EXEC_LINE="./${MINER_DIR}/skypool-node-client --address=\"${WALLET_ADDRESS}\"${CPU_CORES_LINE}${SERVER_URL_DATA}${EXTRADATA}"
  ...
  echo ""
  echo "--------------- Welcome to Skypool.org! Happy mining! ---------------"
  echo ""
fi
